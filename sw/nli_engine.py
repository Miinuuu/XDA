...
def _nli_forward_chunk(
    ...
    if fp16_hw:
        ...
        base_point = pr[index]
        offset = (x_clamped - base_point)  # FP16 subtraction
        ...
        # Fix: when floor_raw >= D_n (address clamped), set frac to 1.0
        # so interpolation reaches the interval boundary value.
        # Do NOT apply to boundary intervals (index==0 or last) where address is forced to 0.
        addr_was_clamped = (floor_val >= D_n) & (index > 0) & (index < num_intervals - 1)
        frac = torch.where(addr_was_clamped, torch.ones_like(frac), frac)
    ...
    # Legacy float32 path used by pre-existing scripts:
    # input is already quantized to FP16 before entering this helper.
...
def _nli_forward_addr_fp32_chunk(
    x_flat: torch.Tensor,
    point_reg: torch.Tensor,
    mul_reg: torch.Tensor,
    lut_reg: torch.Tensor,
    ...
    """Address-path-only FP32 diagnostic.

    Semantics:
      - external input is first quantized to FP16
      - Stage 1/2 address arithmetic uses FP32 point_reg/mul_reg
      - LUT values and interpolation remain FP16
    """
    x_fp16 = x_flat.half().float()
    pr = point_reg.float()
    mr = mul_reg.float()
    lr = lut_reg.half()

    x_clamped = torch.minimum(torch.maximum(x_fp16, pr[0]), pr[M - 1])
    index = torch.bucketize(x_clamped, pr[1:num_intervals])
    ...
    floor_val = torch.floor(scaled_pos)
    address = floor_val.long()
    address = torch.where((index == 0) | (index == num_intervals - 1),
                          torch.zeros_like(address), address)
    address = torch.clamp(address, min=0, max=D_n - 1)

    decimal = torch.where(scaled_pos < 1.0, scaled_pos, scaled_pos - floor_val)
    decimal = torch.clamp(decimal, min=0.0, max=1.0).half()
...
def _nli_forward_full_fp32_chunk(
    x_flat: torch.Tensor,
    point_reg: torch.Tensor,
    mul_reg: torch.Tensor,
    lut_reg: torch.Tensor,
    ...
    """Full FP32 upper-bound path: input, metadata, LUT, and interpolation are FP32."""
    x_f32 = x_flat.float()
    pr = point_reg.float()
    mr = mul_reg.float()
    lr = lut_reg.float()

    x_clamped = torch.minimum(torch.maximum(x_f32, pr[0]), pr[M - 1])
    index = torch.bucketize(x_clamped, pr[1:num_intervals])
    ...
    y = torch.where(x_f32 <= pr[0], lr[0], y)
    y = torch.where(x_f32 >= pr[M - 1], lr[-1], y)
    return y
...
def nli_forward(
    ...
    Variants:
      - fp16_hw: hardware-accurate FP16 path
      - addr_fp32: FP16 I/O + FP32 address path + FP16 interpolation
      - full_fp32: full FP32 upper-bound
      - legacy_float32: pre-existing float path on FP16-quantized input
    """
    if variant is None:
        variant = 'fp16_hw' if fp16_hw else 'legacy_float32'
        ...
        for i in range(0, n, _NLI_CHUNK):
            ...
            if variant == 'fp16_hw':
                parts.append(_nli_forward_chunk(
                    chunk.half().float(), point_reg, mul_reg, lut_reg,
                    D_n, M, num_intervals, fp16_hw=True))
            elif variant == 'legacy_float32':
                parts.append(_nli_forward_chunk(
                    chunk.half().float(), point_reg, mul_reg, lut_reg,
                    D_n, M, num_intervals, fp16_hw=False))
            elif variant == 'addr_fp32':
                parts.append(_nli_forward_addr_fp32_chunk(
                    chunk, point_reg, mul_reg, lut_reg,
                    D_n, M, num_intervals))
            elif variant == 'full_fp32':
                parts.append(_nli_forward_full_fp32_chunk(
                    chunk, point_reg, mul_reg, lut_reg,
                    D_n, M, num_intervals))
            else:
                raise ValueError(f"Unsupported NLI variant: {variant}")
        y = torch.cat(parts, dim=0)
    else:
        if variant == 'fp16_hw':
            y = _nli_forward_chunk(
                x_flat.half().float(), point_reg, mul_reg, lut_reg,
                D_n, M, num_intervals, fp16_hw=True)
        elif variant == 'legacy_float32':
            y = _nli_forward_chunk(
                x_flat.half().float(), point_reg, mul_reg, lut_reg,
            ...
            y = _nli_forward_addr_fp32_chunk(
                x_flat, point_reg, mul_reg, lut_reg,
                D_n, M, num_intervals)
        elif variant == 'full_fp32':
            y = _nli_forward_full_fp32_chunk(
                x_flat, point_reg, mul_reg, lut_reg,
                D_n, M, num_intervals)
        else:
            raise ValueError(f"Unsupported NLI variant: {variant}")
