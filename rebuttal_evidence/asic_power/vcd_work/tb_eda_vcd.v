`timescale 1ns/1ps
// Gate-level VCD-activity testbench for XDA (eda_nli_engine_4s), adapted from
// hw/table1/fpga/tb_eda_power.sv. Same stimulus + func_lut-before-config_rom
// X-prop guard; adds a VCD dump windowed to the steady-state data phase only.
module tb_eda_nli_engine;
    ...
    initial begin
        ...
        // === Data phase: begin continuous VCD here (data-phase activity only,
        //     started post-warmup so cfg-load/reset does not dilute it; no
        //     $dumpoff/$dumpon directives, which OpenSTA's VCD reader rejects) ===
        $dumpfile("eda.vcd");
        $dumpvars(0, u_dut);
        ...
        $display("EDA VCD: %0d vec x %0d rounds, warmup=%0d", num_tests, N_ROUNDS, N_WARMUP);
        $finish;
    end
